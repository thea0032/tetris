...
impl Grid {
    ...
    fn generate_piece(&self) -> Option<Piece> {
        ...
        Some(Piece {
            positions,
            color: Color::Rgb{r: rand::random(), g: rand::random(), b: rand::random()}
        })
    }
    pub fn new(w: usize, h: usize) -> Grid {
        let mut g = Grid {
            filled:vec![vec![false; h]; w],
            color:vec![vec![Color::Green; h]; w],
            piece: Piece::default(),
            rows: h,
            cols: w,
        };
        g.piece = g.generate_piece().expect("If you see this message, the grid is flawed!");
        g
    }
    ...
    fn check_for_lines(&mut self) {
        ...
        for _ in 0..len {
            self.color.insert(0, vec![Color::Green; self.rows]);
